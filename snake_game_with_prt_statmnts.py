...
def update_point(x, y, direction):
    ...
    print("--In function update_point, now coordinate is", str(x) + ',', y)
    return [x, y]

def update_snake(SnakePts, FullGrid, food_coor, direction):
    #import pdb; pdb.set_trace()
    print("--in function update_snake")
    # set head position
    print('----now set head position')
    ...
    print('----head point type is', SnakePts[0].get_snk_type())
    print('----new head point coor is', str(new_coor[0]) + ',', new_coor[1])
    ...
    if meet_food and len(SnakePts) == 1:
        print('----now first-time met food, snake point at', str(head_coor[0]),\
                ',', head_coor[1], 'appended')
        SnakePts.append(Snake(head_coor[0], head_coor[1], 'body'))
        first_time_met_food = True

    # set body position
    if (not first_time_met_food) and (len(SnakePts) != 1):
        print("----now set body position")
        print("----now len of SnakePts is", len(SnakePts))
        for i in range(1, len(SnakePts)):
            print('------now iterating the number', i, 'element of SnakePts')
            # the last snake point
            if i == len(SnakePts)-1:
                # if head meet food
                if meet_food:
                    print('----now meet food, append snake body point after tail')
                    tail_coor = SnakePts[i].get_coordinate()
                    SnakePts.append(Snake(tail_coor[0], tail_coor[1], 'body'))
                    print('----snake point at', str(tail_coor[0]) + ',',\
                            tail_coor[1], 'appended')
    ...
    if SnakePts[0].get_coordinate() == food_coor:
        ...
        while FullGrid[food_coor[1]][food_coor[0]].get_pt_type() == 'snake_head' \
            or FullGrid[food_coor[1]][food_coor[0]].get_pt_type() == 'snake_body':
                food_coor = random_point(GRID_SIZE)
        print('----now regenerating food, coordinate',\
            str(food_coor[0]) + ',', food_coor[1])
    FullGrid[food_coor[1]][food_coor[0]].set_pt_type('food')
    return food_coor
